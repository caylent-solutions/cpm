...
7. Clone package repositories to `.packages/`
8. Create symlinks for shared tooling
9. Make all package targets available

**Important:** All files in `.packages/` and `.repo/` are ephemeral and regenerated by `make configure`. These directories are automatically added to `.gitignore` and should never be committed. Only commit `Makefile` and `.cpmenv` to your repository.
